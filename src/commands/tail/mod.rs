...
pub fn start(
    ...
    let t_port = find_open_port(tunnel_port, DEFAULT_TUNNEL_PORT)?;
    let m_port = find_open_port(metrics_port, DEFAULT_METRICS_PORT)?;
    ...
    Tail::run(target.clone(), user.clone(), t_port, m_port)
}
